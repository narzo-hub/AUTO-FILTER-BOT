   ...
   # if not search:
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            ...
            InlineKeyboardButton("🎭 ᴄʜᴏᴏsᴇ ʟᴀɴɢᴜᴀɢᴇ ✨", callback_data=f"languages#{key}"),
            InlineKeyboardButton("🚩 ʏᴇᴀʀ ⌛", callback_data=f"years#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                ...
                InlineKeyboardButton("😭 ᴇᴘɪsᴏᴅᴇ", callback_data=f"episodes#{key}"),
                InlineKeyboardButton("✨ season🍿",  callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, [
            InlineKeyboardButton("💫 sᴇɴᴅ ᴀʟʟ", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("✨ ᴄʜᴏᴏsᴇ season🍿", callback_data=f"languages#{key}"),
            InlineKeyboardButton("🚩 ʏᴇᴀʀ ⌛", callback_data=f"years#{key}")
        ])
    else:
        btn = []
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton(f'✨ ǫᴜᴀʟɪᴛʏ 🤡', callback_data=f"qualities#{key}"),
                InlineKeyboardButton("😭 ᴇᴘɪsᴏᴅᴇs", callback_data=f"episodes#{key}"),
                InlineKeyboardButton("✨ ᴄʜᴏᴏsᴇ season🍿",  callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("💫 sᴇɴᴅ ᴀʟʟ", callback_data=f"sendfiles#{key}"),
      ...
      #      return
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('ᴜᴘɢʀᴀᴅᴇ 🎟', callback_data='subscription')
            ],[
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', url="https://t.me/+Um_oiYnz7X0wMWY1")
        ],[
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/BruceJaat")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ, ʙᴇʀᴏᴢɢᴀʀ ɪꜱ ᴄʜᴇᴄᴋɪɴɢ ʏᴏᴜʀ ꜱᴘᴇʟʟɪɴɢ...</i></b>")
        movienamelist = []
                    ...
                    if fileid == "None":
                        if btn == "[]":
                            ...
                            try:
                                if settings['auto_ffilter']:
                                    ai_search = True
                                    reply_msg = await message.reply_text(f'🔎 sᴇᴀʀᴄʜɪɴɢ {search}')
                                    await auto_filter(client, message.text, message, reply_msg, ai_search)
                                    try:
                                        if settings['auto_delete']:
                                            await joelkb.delete()
